...
# Test Numba for whole function.
#@njit(parallel = True)  # Throws errors - probably due to use of Xarray argument - maybe just parallelize inner loop...
@jit    # Works, with compilation warnings, but no faster than vanilla calcs.
def MFBLMCalcLoopNumba(matE, eAngs = [0,0,0], thres = 1e-6, p=0, R=0):
    """
    Calculate inner loop for MFBLMs, based on passed set of matrix elements (Xarray).
...
# Test Numba explicitly for main loop (using prange).
@jit(parallel=True)    # Works, with compilation warnings, but no faster than vanilla calcs.
def MFBLMCalcLoopNumbaPrange(matE, eAngs = [0,0,0], thres = 1e-6, p=0, R=0):
    """
    Calculate inner loop for MFBLMs, based on passed set of matrix elements (Xarray).
...
#                print(L)
    ...
    # Generate LM pairs list via indexing
    LMlist = pd.MultiIndex.from_product([matE.SumDim, matE.SumDim], names = ['LM1','LM2'])
    indList = pd.MultiIndex.from_product([np.arange(0, matE.size), np.arange(0, matE.size)], names = ['ind1','ind2']).to_numpy()
    ...
    # Loop over all pairs defined in lists & calculate contributions
    # for rows in indList:
    for nRow in prange(indList.size):
       rows = indList[nRow]
       # print(matE[n[0]], matE[n[1]])
...
# Test Numba explicitly for main loop with prior vectorization of values.
# TODO - needs a careful rewrite, probably best just to start again!
# @jit(parallel=True)    # Works, with compilation warnings, but no faster than vanilla calcs.
# def MFBLMCalcLoopNumbaPreVec(matE, eAngs = [0,0,0], thres = 1e-6, p=0, R=0):
#     """
#     Calculate inner loop for MFBLMs, based on passed set of matrix elements (Xarray).
#
#     Loop code based on Matlab code ePSproc_MFBLM.m
...
#
#     # Loop over all pairs defined in lists & calculate contributions
#     # For numba set to np.array data-types first
#     l1 = int(matE.l.data)
#     m1 = int(matE.m.data)
#     # mu1 = int(matE[r1].mu)  # Case for single mu
#     mu1 = int(matE.mu.data)  # Case for stacked dataArray (mu per item)
#
#     l2 = int(matE.l.data)
#     m2 = int(matE.m.data)
#     # mu2 = int(matE[r2].mu) # Case for single mu
#     mu2 = int(matE.mu.data)  # Case for stacked dataArray (mu per item)
#
#     matEprod = matE[r1] * matE[r2].conj()   # Mat element product
#                                            #TODO: could set additional thresholding here for speed
#
...
def mfblmNumba(da, selDims = {'Type':'L'}, eAngs = [0,0,0], thres = 1e-4, sumDims = ('l','m','mu','Cont','Targ','Total','it')):
    """
    Calculate MFBLMs for a range of (E, sym) cases. Default is to calculated for all symmetries at each energy.
    ...
    if daSumDim.Eke.size > 1:
        for n, daE in daSumDim.groupby('Eke'):
            BLMXlist.append(MFBLMCalcLoopNumba(daE, eAngs = eAngs, thres = thres))

        # Restack along Eke
        BLMXout = xr.combine_nested(BLMXlist, concat_dim=['Eke'])

    else:
        BLMXout = MFBLMCalcLoopNumba(daSumDim, eAngs = eAngs, thres = thres)

    return BLMXout

def mfblmNumbaPrange(da, selDims = {'Type':'L'}, eAngs = [0,0,0], thres = 1e-4, sumDims = ('l','m','mu','Cont','Targ','Total','it')):
    """
    Calculate MFBLMs for a range of (E, sym) cases. Default is to calculated for all symmetries at each energy.
    ...
    if daSumDim.Eke.size > 1:
        for n, daE in daSumDim.groupby('Eke'):
            BLMXlist.append(MFBLMCalcLoopNumbaPrange(daE, eAngs = eAngs, thres = thres))

        # Restack along Eke
        BLMXout = xr.combine_nested(BLMXlist, concat_dim=['Eke'])

    else:
        BLMXout = MFBLMCalcLoopNumbaPrange(daSumDim, eAngs = eAngs, thres = thres)
