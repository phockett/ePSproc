...
def afblmXprod(matEin, QNs = None, AKQS = None, EPRX = None, EPRXresort = None,
                ...
                sumDims = ['mu', 'mup', 'l','lp','m','mp', 'S-Rp'], sumDimsPol = ['P','R','Rp','p','R-p'], symSum = True,  # Fixed summation ordering for AF*pol term...?
                outputDims = {'LM':['L','M']},
    ...
    23/03/24    Updating for arb pol case (p!=0), changes as per working mfblmGeom.py 05/03/24
                    - Allow passing of `ep`.
                    - Allow direct passing for EPRXresort (skips default).
                    - Updated defaults for p, ep.
                    - Default case still p=0, now set via ep=[0,1,0] selection.
                    - Other defaults based on inputs, see p, ep help below.
                    - Added 'R-p' to sumDimsPol defaults.
                    - Added 'polProd' tp full basis return dict.
    ...
    *** Optional input data/basis functions (mainly for fitting routine use)
    ...
    EPRX, EPRXresort : Xarray, optional, default = None
    ...
    p : list or array, optional, default = None
        ...
        UPDATE 23/03/24: now supports all p terms via ep values, default p=[-1,0,1]

    ep : list or array, optional, default = None
        Set E-field components.
        If p and ep are None, will default to ep=[0,1,0] for p=0 term only.
        If p is set, but ep=None, will default to ep=np.ones(len(p)).
    ...
    **kwargs, unused but allows for arb basis dict unpack and passing from other functions.
    ...
    # 23/03/24: for updated EPR including ep terms.
    # Note passing ep=None to EPR() will set ep=np.ones(len(p))
    # If p is not passed, set for all allowed values, but limit to p=0 only (to match previous code)
    if p is None:
        p = [-1,0,1]

        # If no field strength components are defined, set to p=0 term only (matches pre-ep defaults)
        if ep is None:
            ep = np.array([0,1,0])
...
# 23/03/24 case with ep configured for all p.
        EPRX = geomCalc.EPR(form = 'xarray', p = p, ep = ep).unstack()  # 06/03/24: for updated EPR including ep terms.

    if (EPRXresort is None) and (polProd is None):  # Skip if product term already passed
...
#     BLMXout = BLMXout/BLMXout.XS  # Normalise
    ...
    # Return basis arrays/tensors
        ...
        basis = {'QNs':QNs, 'EPRX':EPRXresort, 'lambdaTerm':lambdaTermResort,
                ...
                'matEmult':matEmult, 'polProd':polProd}   #, 'phaseCons':phaseCons}
