...
Main function ePSread(fileIn = None, fileBase = None, fType = '.out'):
...
def parseDumpIDY(dumpSeg):
    ...
    >>> matEle, attribs = parseDumpIDY(dumpSegs[0])
...
#        rawIdy.append(lineOut)
    ...
    attribs.append(['E', np.float(parseDigits(dumpSeg[3][2])[0]), 'eV'])
    attribs.append(['Ehv', np.float(parseDigits(dumpSeg[4][2])[0]), 'eV'])
    SF = np.genfromtxt(parseDigits(dumpSeg[5][2]))
    SF = SF[0] + SF[1]*1j
    attribs.append(['SF', SF, 'sqrt(MB)'])
    attribs.append(['Lmax', np.int(parseDigits(dumpSeg[10][2])[0]), ''])

    # Parse symmetries - multiple .spilt(), or use re?
    # attribs.append(['Syms', parseDigits(dumpSeg[7][2]), ''])
...
def parseDumpSegsX(dumpSegs):
    ...
    for dumpSeg in dumpSegs:
        if len(dumpSeg)>4:
            segBlock, attribs = parseDumpIDY(dumpSeg)
...
def ePSread(fileIn = None, fileBase = None, fType = '.out'):
    """
    ...
    >>> dataSet = ePSread()  # Scan current dir

    >>> fileIn = r'C:\share\code\ePSproc\python_dev\no2_demo_ePS.out'
    >>> dataSet = ePSread(fileIn)  # Scan single file

    >>> dataSet = ePSread(fileBase = r'C:\share\code\ePSproc\python_dev') # Scan dir
    ...
    for file in fList:
        ...
        #lines, dumpSegs = ePSIdyfileParse(os.path.join(fileBase, file))
        lines, dumpSegs = ePSIdyfileParse(file)
        data, blankSegs = parseDumpSegsX(dumpSegs)
